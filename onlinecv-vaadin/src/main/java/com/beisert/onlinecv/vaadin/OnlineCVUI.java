...
public class OnlineCVUI extends UI {
	...
	Button downloadButton = new Button("Download",FontAwesome.FILE_PDF_O);
	...
	private void initEditor() {
		...
		buttonRow.addComponents(addNewCVButton,saveCVButton,spacer, cancelButton, downloadButton);
	...
	private void initButtons() {
		...
		this.downloadButton.addClickListener(evt -> {
			OnlineCV cv = (OnlineCV) this.beanForm.getBean();
			if(cv == null) return;
			String user = cv.getUser();
			String url = "http://localhost:8090/run?__report=cv_report.rptdesign&__format=pdf&resturl=http://localhost:8888/rest/onlinecv/"+user+"?format=xml";
			Page.getCurrent().open(url, "Report");
		});
